...
public class Render {
	...
	private int myX = 480, myY = 700;
	private int rightX = 1400, rightY = 200;
	private int leftX = 100, leftY = 200;
	private int pokeSpan = 30, selectedSpan = 20;
	private int pokeWidth = 105, pokeHeight = 150;
	...
	public Render(JPanel panel, int width, int height) {
		this.panel = panel;
		...
		myX = width / 2 - 300;
		myY = height - 100 - pokeHeight;

		leftX = 100;
		leftY = 100;

		rightX = width - 100 - pokeWidth;
		rightY = 100;
		// rightX = 200;
		// rightY = 100;
	}
	...
	public void drawMyself(Graphics g, List<Card> cards, boolean[] selected) {
		int size = cards.size() >> 1;
		myX = width / 2 - size * pokeSpan - 60;
		int x = myX, y = myY;
		int a = 0, i = 0;
		for (Card c : cards) {
			a = (c.getKey() - 1) * 4;
			...
			g.drawString("myX:" + myX + ",myY:" + myY, myX - 10, myY);
			if (selected[i++]) {
